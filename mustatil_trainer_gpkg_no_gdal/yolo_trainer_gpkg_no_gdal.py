...
def open_large_image_info(path: Path, log_fn=None):
    ...
    mode='rasterio' streams huge GeoTIFFs.
    mode='pil' is fallback for normal images.
    """
    if is_probably_geotiff(path) and rasterio_available():
        try:
            import rasterio
            ...
            if log_fn:
                log_fn(f"Rasterio streaming open failed, falling back to PIL: {exc}")

    im = Image.open(path)
    if log_fn:
        log_fn("Large image mode: PIL full-image mode")
...
def _epsg_int(crs_name):
    try:
        ...
        if "EPSG:" in s:
            return int(s.split("EPSG:")[-1].split()[0].replace('"', '').replace("'", ""))
        return int(s)
    except Exception:
        return 3857
...
def _polygon_wkb(poly_coords):
    """
    Writes simple 2D Polygon WKB, little endian.
    poly_coords: GeoJSON polygon coordinates: [ring1, ring2, ...]
    """
    import struct
    ...
    out = bytearray()
    out += struct.pack("<B", 1)          # little endian
    out += struct.pack("<I", 3)          # WKB Polygon
    out += struct.pack("<I", len(rings)) # ring count
...
def _gpkg_geom_blob(wkb, srs_id):
    """
    GeoPackage geometry BLOB:
    'GP' magic, version 0, flags 1 little-endian no envelope, srs_id, then WKB.
    """
    import struct
...
def write_features_outputs(out_path: Path, feature_collection: dict, crs_name: Optional[str], log_fn=None):
    """
    Writes detection results.

    Supported:
    - .geojson/.json
    - .gpkg without geopandas/fiona/GDAL, using only sqlite3 stdlib

    This avoids Fiona/GDAL installation errors on Windows/Anaconda.
    """
    ...
    if suffix == ".gpkg":
        ...
        # CRS sanity: WebMercator meter coords are not EPSG:4326 degrees.
        try:
            first = feature_collection["features"][0]["geometry"]["coordinates"][0][0]
        ...
        cur.execute("""
            INSERT INTO gpkg_spatial_ref_sys
            VALUES ('WGS 84 geodetic', 4326, 'EPSG', 4326, 'EPSG:4326', 'longitude/latitude coordinates in decimal degrees on WGS84')
        """)
        cur.execute("""
            INSERT INTO gpkg_spatial_ref_sys
            VALUES ('WGS 84 / Pseudo-Mercator', 3857, 'EPSG', 3857, 'EPSG:3857', 'Web Mercator meters')
        """)
        ...
        # Feature table. Keep props useful but simple.
        cur.execute("""
        ...
        cur.execute("""
            ...
            VALUES ('mustatil_detections', 'geom', 'POLYGON', ?, 0, 0)
        """, (srs_id,))
        ...
        for feat in feature_collection.get("features", []):
            ...
            if not wkb:
                continue
            blob = _gpkg_geom_blob(wkb, srs_id)
            ...
            cur.execute("""
                ...
                (geom, class, class_id, confidence, geometry_source, backend, georef_source, image_read_mode, pixel_bbox)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                sqlite3.Binary(blob),
                props.get("class"),
        ...
        # Optional RTree extension is not required for QGIS to display the layer.
        conn.commit()
        conn.close()

        if log_fn:
            log_fn(f"GeoPackage written without GDAL/Fiona/GeoPandas: {out_path}")
            log_fn(f"GeoPackage features: {count}; EPSG:{srs_id}; bounds=({minx}, {miny}, {maxx}, {maxy})")
...
class MustatilGUI(tk.Tk):
    ...
    def _build_detect_tab(self):
        ...
        rows = [
            ...
            ("Output GeoPackage/GeoJSON", self.out_geojson, self.pick_output_geojson),
        ]
    ...
    def redraw_detection_preview(self):
        ...
        scale_canvas = min(cw / im.width, ch / im.height, 1.0)
        ...
        for feat in self.det_preview_features:
            ...
            if source == "sam2_mask" and poly_px and len(poly_px) >= 3:
                ...
                for p in poly_px:
                    if len(p) >= 2:
                        pts.extend([ox + float(p[0]) * total_scale_x, oy + float(p[1]) * total_scale_y])
                if len(pts) >= 6:
        ...
        self.det_canvas.create_text(
            ...
            anchor="nw",
            fill="white",
            text=f"Preview: {im.width}x{im.height} | original: {orig_w}x{orig_h} | features: {features_count} | SAM2 masks: {sam_shown} | boxes: {bbox_shown}",
        )
    ...
    def detect(self):
        ...
        self.log_detect(f"FINAL: backend={backend}; read_mode={mode}; georef={georef_kind}; YOLO boxes={yolo_detections}; SAM2 masks={sam_success}; fallback boxes={sam_fallback}")
