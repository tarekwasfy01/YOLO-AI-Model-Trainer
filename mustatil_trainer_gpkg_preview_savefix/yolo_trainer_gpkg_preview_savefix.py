...
def _write_gpkg_sqlite(gpkg_path: Path, feature_collection: dict, srs_id: int, log_fn=None):
    ...
    try:
        ...
        insert_sql = """
            ...
            (geom, class, class_id, confidence, geometry_source, backend, georef_source, image_read_mode, inference_engine, pixel_bbox)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
...
def write_features_outputs(out_path: Path, feature_collection: dict, crs_name: Optional[str], log_fn=None):
    ...
    if suffix == ".gpkg":
        ...
        # 1) Preview first, so the app preview works even if QGIS locks the gpkg.
        preview_path = preview_geojson_path_for_output(out_path)
        try:
            preview_path.write_text(json.dumps(feature_collection, indent=2), encoding="utf-8")
            if log_fn:
                log_fn(f"Preview GeoJSON written first: {preview_path}")
        except Exception as exc:
            if log_fn:
                log_fn(f"Preview GeoJSON write failed: {exc}")
...
class MustatilGUI(tk.Tk):
    ...
    def load_detection_preview(self, image_only=False):
        ...
        if not image_only:
            ...
            gj_path = preview_geojson_path_for_output(raw_out)
            if gj_path.exists():
                try:
    ...
    def detect(self):
        ...
        try:
            ...
            self.log_detect("Preview reloaded from .preview.geojson sidecar.")
        except Exception as exc:
