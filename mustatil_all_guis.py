...
class GUI(tk.Tk):
    ...
    def build(self):
        ...
        # Compute-Confidence ist ein eigenes Eingabefeld und is nur beim YOLO-Rechnen benutzt.
        # Display-Confidence ist nur ein Preview-Filter und ändert keine gespeicherten/berechneten Ergebnisse.
        ...
        ttk.Label(ann_left,text='Annotationen dieses Imagees').pack(anchor='w')
        ...
        sam_box=ttk.LabelFrame(ann_left,text='SAM2 für aktuelles Crop / Project',padding=5); sam_box.pack(fill=tk.X,pady=6)
        ...
        ttk.Button(sam_box,text='SAM2: alle Crops im Project segmentieren',command=lambda:threading.Thread(target=self.sam2_all_annotation_project,daemon=True).start()).grid(row=4,column=0,columnspan=3,sticky='ew',pady=2)
        ...
        ttk.Label(sam_left,text='Imageordner für SAM2').pack(anchor='w')
        ...
        ttk.Button(sam_left,text='aus Project/images übernehmen',command=self.sam_use_project_images).pack(fill=tk.X,pady=2)
        ttk.Label(sam_left,text='Alle images/Crops').pack(anchor='w',pady=(8,0))
        ...
        ttk.Checkbutton(settings,text='Batch: bereits segmentierte images überspringen (.sam2.json vorhanden)',variable=self.sam_skip_existing).grid(row=3,column=0,columnspan=2,sticky='w')
        ...
        ttk.Button(sam_left,text='SAM2: ausgewähltes Image segmentieren',command=lambda:threading.Thread(target=self.sam2_selected,daemon=True).start()).pack(fill=tk.X,pady=4)
        ttk.Button(sam_left,text='SAM2: alle images segmentieren',command=lambda:threading.Thread(target=self.sam2_all,daemon=True).start()).pack(fill=tk.X,pady=4)
        ttk.Label(sam_left,text='Hinweis: Wenn YOLO-Labels vorhanden sind, werden diese Boxen als Prompts genutzt. Sonst is das ganze Image als Prompt segmentiert.').pack(anchor='w',pady=4)
        ...
        ttk.Label(fl_left,text='Nutzt Modele/Image/Tile/Overlap aus Tab 1 und filtert anschließend mit FormLearner.').pack(anchor='w')
        ...
        ttk.Button(fl_left,text='Preview neu zeichnen',command=self.draw_fl_preview).pack(fill=tk.X,pady=2)
    ...
    def create_new_project_folder_dialog(self, target_var=None):
        ...
        try:
            ...
            self.log('Create new folder error '+str(e)); messagebox.showerror(APP,str(e))
    ...
    def create_project_structure_from_detection(self):
        try:
            ...
            self.log('Create project error '+str(e)); messagebox.showerror(APP,str(e))
    ...
    def create_crop_annotator_image_folder(self):
        try:
            ...
            self.log('Create image folder error '+str(e)); messagebox.showerror(APP,str(e))
    ...
    def detect(self):
        try:
            ...
            self.score(); self.log(f'Detection finished: {len(self.dets)} Treffer'); self.redraw()
    ...
    def showann(self):
        if not self.ann_imgs:
            if hasattr(self,'ann_status'): self.ann_status.set('Keine images im Projectordner/images gefunden.')
            return
    ...
    def load_sam_images(self):
        ...
        if not folder.exists():
            messagebox.showerror(APP,f'Imageordner nicht gefunden: {folder}')
            return
        ...
        self.sammsg(f'SAM2 image list geladen: {len(self.sam_images)} images aus {folder}')
        if self.sam_images:
        ...
        try:
            for i,bb in enumerate(boxes,1):
                ...
                if i%5==0 or i==len(boxes):
                    self.sammsg(f'SAM2 Progress {img.name}: {i}/{len(boxes)} | Masken={len(polys)}')
        finally:
            try:
                if mode=='rasterio': reader.close()
            except Exception: pass
    ...
    def sam2_selected(self):
        try:
            if not getattr(self,'sam_selected_img',None):
                self.load_sam_images()
            if not getattr(self,'sam_selected_img',None):
                raise RuntimeError('Kein Image ausgewählt.')
            ...
            self.draw_sam_preview()
            self.sammsg(f'SAM2 fertig für ausgewähltes Image: {Path(self.sam_selected_img).name}')
        except Exception as e:
    ...
    def sam2_all(self):
        try:
            ...
            if not self.sam_images: raise RuntimeError('Keine images in der SAM2-Liste.')
            sam=load_sam2_model_safe(self.sammodel.get().strip() or 'sam2_b.pt', log_fn=self.sammsg)
            all_polys=[]
            skipped=0
            for idx,img in enumerate(self.sam_images,1):
                ...
                if self.sam_skip_existing.get() and imgp.with_suffix('.sam2.json').exists():
                    ...
                    continue
                self.sammsg(f'=== SAM2 alle images: {idx}/{len(self.sam_images)} ===')
            ...
            if skipped:
                self.sammsg(f'SAM2 Batch: bereits segmentierte images übersprungen: {skipped}')
            ...
            self.sammsg(f'SAM2 alle images fertig: {len(all_polys)} Masken | Sammeldatei: {out}')
        except Exception as e:
    ...
    def sam2_current_annotation(self):
        try:
            if not self.ann_imgs:
                raise RuntimeError('Kein Crop-Project geladen.')
    ...
    def sam2_all_annotation_project(self):
        try:
            if not self.ann_imgs:
                self.loadproj()
            if not self.ann_imgs:
                raise RuntimeError('Keine Crops im Project geladen.')
            sam=load_sam2_model_safe(self.sammodel.get().strip() or 'sam2_b.pt', log_fn=self.sammsg)
            skipped=0
            for i,img in enumerate(self.ann_imgs,1):
                ...
                if self.sam_skip_existing.get() and imgp.with_suffix('.sam2.json').exists():
                    skipped += 1
                    self.sammsg(f'SAM2 Project-Crop übersprungen, schon vorhanden: {i}/{len(self.ann_imgs)} {imgp.name}')
                    continue
                self.sammsg(f'SAM2 Project-Crops: {i}/{len(self.ann_imgs)} {imgp.name}')
                self.sam2_segment_image(imgp,sam=sam,save=True)
            if skipped:
                self.sammsg(f'SAM2 Project-Crops übersprungen: {skipped}')
            self.showann()
            self.sammsg('SAM2 für alle Project-Crops fertig.')
        except Exception as e:
    ...
    def _dets_to_features(self, dets):
        ...
        for i,d in enumerate(dets,1):
            ...
            feats.append({'type':'Feature','geometry':{'type':'Polygon','coordinates':[[(x1,y1),(x2,y1),(x2,y2),(x1,y2),(x1,y1)]]},'properties':props})
        return feats
    ...
    def _write_features_auto(self, out, feats, layer='detections', log_fn=None):
        ...
        if out.suffix.lower()=='.gpkg':
            try:
                write_gpkg_fallback(out, feats, layer=layer, crs_name=None, log_fn=log_fn or self.log)
            except Exception as e:
    ...
    def export_geo_by_model(self):
        ...
        try:
            ...
            self.log('Per-model export error '+str(e)); messagebox.showerror(APP,str(e))
    ...
    def export_formlearner_current(self):
        ...
        try:
            ...
            for rank,(d,fs) in enumerate(kept,1):
                x1,y1,x2,y2=d.bbox()
                feats.append({'type':'Feature','geometry':{'type':'Polygon','coordinates':[[(x1,y1),(x2,y1),(x2,y2),(x1,y2),(x1,y1)]]},'properties':{'rank':rank,'model':d.name,'model_index':d.slot+1,'confidence':float(d.conf),'ensemble_score':float(getattr(d,'score',0.0)),'consensus':int(getattr(d,'consensus',1)),'form_score':float(fs),'formscore_threshold':thr,'pixel_bbox':[x1,y1,x2,y2]}})
            ...
            self.log('FormLearner export error '+str(e)); messagebox.showerror(APP,str(e))
    ...
    def export_geo(self):
        try:
            ...
            self.log('Export error '+str(e)); messagebox.showerror(APP,str(e))
    ...
    def train_formlearner(self):
        try:
            ...
            if not imgs_dir.exists(): raise RuntimeError('Projectordner braucht images/')
            if not labs_dir.exists(): raise RuntimeError('Projectordner braucht labels/')
            X=[]; y=[]; nimg=0
            ...
            self.flog(f'FormTrainer samples: {len(y)} | positivee={pos} | false={neg} | images={nimg}')
